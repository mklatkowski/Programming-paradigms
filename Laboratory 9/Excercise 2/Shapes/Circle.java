...
public class Circle extends Shape implements Mixin{

    private int radius;
    private static Circle instance = null;
    Circle(int radius, boolean filled, int x, int y){
        super(filled);
        if(radius<=0){
    ...
    public static Circle getInstanceOf(int radius, int x, int y, boolean filled){
        if(instance == null){
            instance = new Circle(radius, filled, x, y);
        }
        else{
            instance.radius = radius;
            instance.filled = filled;
            instance.point.setX(x);
            instance.point.setY(y);
        }
        return instance;
    }
